...
const trackRef = useTemplateRef<any>('trackRef');
...
<template>
  ...
  >
    ...
    <div ref="trackRef" class="flex w-max will-change-transform select-none motion-reduce:transform-none">
      ...
      >
        <li
          v-for="(logo, logoIndex) in props.logos"
          :key="`${copyIndex - 1}-${logoIndex}`"
          class="flex-none mr-(--logoloop-gap) text-(length:--logoloop-logoHeight) leading-none" :class="[
        ...
        >
          ...
          >
            <LogoContent :item="logo" :scale-on-hover="props.scaleOnHover" />
          </a>
          <LogoContent v-else :item="logo" :scale-on-hover="props.scaleOnHover" />
        </li>
      </ul>
    </div>
  </div>
